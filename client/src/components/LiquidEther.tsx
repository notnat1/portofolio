...
class Simulation {
  ...
  constructor(common: CommonClass, mouse: MouseClass, options?: Partial<SimOptions>) { // Accept common and mouse
    this.Common = common;
    this.Mouse = mouse;
    this.options = {
  ...
  createShaderPass() {
    this.advection = new Advection({
      ...
      src: this.fbos.vel_0,
      dst: this.fbos.vel_1
    }, this.Common); // Pass common
    this.externalForce = new ExternalForce({
      cellScale: this.cellScale,
      cursor_size: this.options.cursor_size,
      dst: this.fbos.vel_1
    }, this.Common, this.Mouse); // Pass common and mouse
...
class Output {
  ...
  constructor(common: CommonClass, mouse: MouseClass, paletteTex: THREE.DataTexture) { // Accept common, mouse, paletteTex
    this.Common = common;
    this.simulation = new Simulation(common, mouse); // Pass common and mouse
    this.scene = new THREE.Scene();
    this.camera = new THREE.Camera();
...
class WebGLManager implements LiquidEtherWebGL {
  ...
  constructor(props: any, common: CommonClass, mouse: MouseClass, paletteTex: THREE.DataTexture) { // Accept common, mouse, paletteTex
    ...
    this.init(paletteTex); // Pass paletteTex
  ...
  init(paletteTex: THREE.DataTexture) { // Accept paletteTex
    if (!this.Common.renderer) return;
    this.props.$wrapper.prepend(this.Common.renderer.domElement);
    this.output = new Output(this.Common, this.Mouse, paletteTex); // Pass common, mouse, paletteTex
  }
  resize() {
    this.Common.resize();
    this.output.resize();
...
let isVisibleRef: React.MutableRefObject<boolean> = { current: true };
let rafRef: React.MutableRefObject<number | null> = { current: null };
...
export default function LiquidEther({
  ...
  const localIsVisibleRef = useRef<boolean>(true);
  isVisibleRef = localIsVisibleRef;
  ...
  useEffect(() => {
    ...
    // Instantiate Common and Mouse here
    ...
    const webgl = new WebGLManager({
      ...
      autoResumeDelay,
      autoRampDuration
    }, Common, Mouse, paletteTex); // Pass Common, Mouse, and paletteTex
    ...
    const io = new IntersectionObserver(
      entries => {
        ...
        localIsVisibleRef.current = isVisible;
        if (!webglRef.current) return;
